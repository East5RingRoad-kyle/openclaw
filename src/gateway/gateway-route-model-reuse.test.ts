  ...
  it(
    ...
    ({ signal }) =>
      fixture.run(async () => {
        ...
        // Stop pending RPCs on cancellation; the ordered cleanup below joins that stop.
        const stopClient = () => client?.stop();
        signal.addEventListener("abort", stopClient, { once: true });
        const proof = await runQaGatewayFixture(
          async () => {
            ...
            const stats = () => {
              signal.throwIfAborted();
              return activeClient.request<{ counts: Counts; reloadSettled: boolean }>(
